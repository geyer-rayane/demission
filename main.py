"""
Pipeline principal — analyse après nettoyage
1. Nettoyage des données brutes
2. Analyse univariée des tables nettoyées
3. Analyse bivariée des tables nettoyées
"""
...
DATA_DIR     = "data"
OUTPUT_ROOT  = "analyse_apres_nettoyage"

TABLE1_CLEAN = os.path.join(DATA_DIR, "table1_clean.csv")
TABLE2_CLEAN = os.path.join(DATA_DIR, "table2_clean.csv")
...
assert os.path.exists("nettoyage.py"), "[ERREUR] nettoyage.py introuvable"
...
print("=" * 70)
result = subprocess.run(["python", "nettoyage.py"], capture_output=False)
if result.returncode != 0:
    raise RuntimeError("[ERREUR] nettoyage.py a échoué.")
print("[OK] Nettoyage terminé\n")

# ── Chargement ────────────────────────────────────────────────────────────────

print("Chargement des données nettoyées...")
table1 = pd.read_csv(TABLE1_CLEAN)
table2 = pd.read_csv(TABLE2_CLEAN)
